...
class UserManager(BaseUserManager): # subclass of BaseUserManager - will override some functions to handle email address instead of username
    ...
    # **extra_fields <-- takes any extra functions that are passed in and pass  them into extra fields so we can just add any additional fields that we use with our user models
